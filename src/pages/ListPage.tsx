...
/**
 * Document root for `/`. Owns the embedded listings payload, the shared clock,
 * and geolocation, then renders the list UI.
 */
...
function ListView({
  ...
  // back-nav that didn't hit bfcache) — the query is applied post-hydration by
  // useUrlParams.
  useEffect(() => {
  ...
  // No manual scroll save/restore: list↔detail are separate documents, so the
  // browser restores list scroll natively on back/swipe-back (bfcache).
